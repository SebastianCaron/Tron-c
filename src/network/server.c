...
server *init_serveur(int port, int nb_connect){
    ...
    if (uni_set_sockopt(server_fd) < 0) {
    ...
    if (uni_listen(server_fd, MAX_CLIENT) < 0) {
...
int wait_for_connections(server *s, void (*on_connect)(char *)){
    ...
    if(new_socket != 0){
        ...
        uni_write(new_socket, (char *) buffer, 3 * sizeof(int));
        s->act_connect++;
        return 1;
    }
    return 0;
...
int wait_for_connections_timeout(server *s, void (*on_connect)(char *)) {
    ...
    int activity = uni_select(s, &readfds, &timeout);
    if (activity < 0) {
    ...
    if (FD_ISSET(s->serveur_fd, &readfds)) {
        ...
        s->clients_fd[s->act_connect] = new_socket;
        int buffer[3] = {IDSERV, s->act_connect, ENDPACKET};
        if (uni_write(new_socket, (char *)buffer, 3 * sizeof(int)) < 0) {
            perror("[SERVER] Erreur lors de l'écriture sur le socket client");
            uni_close(new_socket);
            return -1;
        }
        s->act_connect++;
...
void send_grid_to(server *s, int connect, int nb_lignes, int nb_colonnes, int **grid){
    ...
    uni_write(s->clients_fd[connect], buffer, size);
    free(buffer);
}
...
void send_is_over_to(server *s, int connect, int est_fini){
    ...
    uni_write(s->clients_fd[connect], (char *) buffer, size);
}
...
void retrieve_data_server(server *s){
    ...
    for(int i = 0; i < s->nb_connect-1; i++){
        int rd_size = uni_read(s->clients_fd[i], &f_type, sizeof(char));
        // printf("READ : %d, NB :%d\n", f_type, rd_size);
        if(rd_size == 0) return;
        if(rd_size == -1){
        ...
        case MOUVEMENT:
            ...
            char buffer[3] = {0};
            rd_size = uni_read(s->clients_fd[i], buffer, size);
            while(rd_size < size){
                rd_size += uni_read(s->clients_fd[i], ((char *) buffer) + rd_size, size);
            }
        ...
        case GRID:
            ...
            char buffer[2] = {0};
            rd_size = uni_read(s->clients_fd[i], ((char *) buffer), size);
            while(rd_size < size){
                rd_size += uni_read(s->clients_fd[i], ((char *) buffer) + rd_size, size);
            }
...
void send_grid_all(server *s, int nb_lignes, int nb_colonnes, int **grid){
    ...
    for(int i = 0; i < s->nb_connect; i++){
        uni_write(s->clients_fd[i], buffer, size);
    }
    free(buffer);
}
